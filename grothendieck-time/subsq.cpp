...
int solve(int pos, int prev, int cnt)
{
    ...
    if (a[pos] != prev)
    {
        val = max(val, 1 + solve(pos + 1, a[pos], cnt + 1));
    }
    else if (a[pos] == prev)
    {   
        val = max(val, 1 + solve(pos + 1, a[pos], cnt));
    }

    val = max(val, solve(pos + 1, prev, cnt));

    return dp[pos][prev][cnt] = val;
}
