...
signed main()
{
    ...
    while (t--)
    {
        ...
        int left = x1, right = x - x2, up = y - y2, down = y1;
        
        ld ans = INF, mx = 0, my = 0;
        
        //left
        mx = max((ld)0, (ld)a - left);
        if (x2 + mx <= x)
            ans = min(ans, mx);

        //right
        mx = max((ld)0, (ld)a - right);
        if (x1 - mx >= 0)
            ans = min(ans, mx);

        //down
        my = max((ld)0, (ld)b - down);
        if (y2 + my <= y)
            ans = min(ans, my);
        ...
        if (ans == INF)
            ...
            cout << setprecision(9) << (ld)ans << endl;
    }
    return 0;
}
